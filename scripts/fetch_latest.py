#!/usr/bin/env python3
"""Fetch latest STAT6 items from PubMed and ClinicalTrials.gov, update data/updates.json,
and regenerate the "最新进展" block inside research.html.

Runs daily via .github/workflows/stat6-daily-update.yml.
...
USER_AGENT = "STAT6-InfoSite-DailyBot/1.0 (+https://github.com/suguizhuan/sugz)"
...
def fetch_pubmed(days: int = 2, max_items: int = 15) -> list[dict]:
    """Return PubMed articles mentioning STAT6 in title/abstract, updated in the last `days`."""
    try:
        search = http_get_json(
            PUBMED_ESEARCH,
            {
                "db": "pubmed",
                "term": "STAT6[Title/Abstract]",
                "reldate": str(days),
                "datetype": "pdat",
                "sort": "date",
...
def fetch_clinicaltrials(max_items: int = 10) -> list[dict]:
    """Return recently updated ClinicalTrials.gov studies mentioning STAT6."""
    try:
        params = {
            "query.term": "STAT6",
